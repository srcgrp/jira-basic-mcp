...
if (!JIRA_HOST || !JIRA_EMAIL || !JIRA_API_TOKEN) {
  console.error('Error: JIRA_HOST, JIRA_EMAIL, and JIRA_API_TOKEN environment variables are required.');
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        assignee: { type: 'string', description: 'Account ID of the new assignee (use get_user to find)' },
        status: { type: 'string', description: 'New status name (requires transition ID internally)' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        assignee: { type: 'string', description: 'Account ID of the assignee (use get_user to find)' },
...
server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
  ...
  console.error(`Received call for tool: ${name} with args:`, JSON.stringify(args, null, 2));
  ...
  if (validator && !validator(args)) {
    console.error(`Invalid arguments for tool ${name}:`, ajv.errorsText(validator.errors));
    throw new McpError(
    ...
    switch (name) {
      ...
      case 'create_issue': {
        ...
        if (!foundType || !foundType.id) {
          throw new McpError(ErrorCode.InvalidParams, `Issue type "${issueType}" not found.`);
        }
        ...
        if (components && components.length > 0 && project.id) {
          try {
            const projectComponents = await jiraClient.projectComponents.getProjectComponents({ projectIdOrKey: project.id });
            componentObjects = components
              ...
              .filter((comp: { id: string } | null): comp is { id: string } => comp !== null);
          } catch (e: any) {
      ...
      default:
        throw new McpError(ErrorCode.MethodNotFound, `Handler for tool "${name}" not implemented.`);
    }
